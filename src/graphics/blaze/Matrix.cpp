...
Matrix::Matrix(const Matrix& matrix1, const Matrix& matrix2) {
    m[0][0] = matrix2.m[0][0] * matrix1.m[0][0] + matrix2.m[0][1] * matrix1.m[1][0];

    m[0][1] = matrix2.m[0][0] * matrix1.m[0][1] + matrix2.m[0][1] * matrix1.m[1][1];

    m[1][0] = matrix2.m[1][0] * matrix1.m[0][0] + matrix2.m[1][1] * matrix1.m[1][0];

    m[1][1] = matrix2.m[1][0] * matrix1.m[0][1] + matrix2.m[1][1] * matrix1.m[1][1];

    m[2][0] = matrix2.m[2][0] * matrix1.m[0][0] + matrix2.m[2][1] * matrix1.m[1][0] + matrix1.m[2][0];

    m[2][1] = matrix2.m[2][0] * matrix1.m[0][1] + matrix2.m[2][1] * matrix1.m[1][1] + matrix1.m[2][1];
}
...
Matrix Matrix::Lerp(const Matrix& matrix1, const Matrix& matrix2, const Float t) {
...
FloatRect Matrix::Map(const FloatRect& rect) const {
    ...
    const Float minX             = Min(topLeft.X, Min(topRight.X, Min(bottomLeft.X, bottomRight.X)));
    const Float maxX             = Max(topLeft.X, Max(topRight.X, Max(bottomLeft.X, bottomRight.X)));
    const Float minY             = Min(topLeft.Y, Min(topRight.Y, Min(bottomLeft.Y, bottomRight.Y)));
    const Float maxY             = Max(topLeft.Y, Max(topRight.Y, Max(bottomLeft.Y, bottomRight.Y)));
...
bool Matrix::IsIdentity() const {
    ...
    return m[0][0] == 1 && m[1][1] == 1 && m[0][1] == 0 && m[1][0] == 0 && m[2][0] == 0 && m[2][1] == 0;
}

bool Matrix::IsEqual(const Matrix& matrix) const {
    return FuzzyIsEqual(m[0][0], matrix.m[0][0]) && FuzzyIsEqual(m[0][1], matrix.m[0][1]) &&
           FuzzyIsEqual(m[1][0], matrix.m[1][0]) && FuzzyIsEqual(m[1][1], matrix.m[1][1]) &&
           FuzzyIsEqual(m[2][0], matrix.m[2][0]) && FuzzyIsEqual(m[2][1], matrix.m[2][1]);
}
...
MatrixComplexity Matrix::DetermineComplexity() const {
    ...
    const int mask = (int(translation) << 2) | (int(scale) << 1) | (int(complex) << ComplexBit);
