...
static void constrainMaxSizeForMode(
  ...
  const FloatOptional maxSize =
      ...
      FloatOptional(node->style().computeMarginForAxis(
          axis, ownerWidth)); // content-box: add paddingAndBorder for axis
  switch (*mode) {
...
static void computeFlexBasisForChild(
  ...
  float childWidth = undefined;
  float childHeight = undefined;
  ...
  if (resolvedFlexBasis.isDefined() && yoga::isDefined(mainAxisSize)) {
    if (child->getLayout().computedFlexBasis.isUndefined() ||
        (isExperimentalFeatureEnabled(ExperimentalFeature::WebFlexBasis) &&
    ...
    if (isRowStyleDimDefined) {
      childWidth = child->getResolvedDimension(Dimension::Width)
                       .resolve(ownerWidth)
                       .unwrap() +
          marginRow; // content-box: add paddingAndBorder for axis
    ...
    if (isColumnStyleDimDefined) {
      childHeight = child->getResolvedDimension(Dimension::Height)
                        .resolve(ownerHeight)
                        .unwrap() +
          marginColumn; // content-box: add paddingAndBorder for axis
      childHeightSizingMode = SizingMode::StretchFit;
    }
    const bool scrollRow =
        (node->style().overflow() & Overflow::AllowRow) == Overflow::AllowRow;
    const bool scrollColumn = (node->style().overflow() &
                               Overflow::AllowColumn) == Overflow::AllowColumn;
...
static void measureNodeWithMeasureFunc(
  ...
  if (widthSizingMode == SizingMode::StretchFit &&
    ...
    const Size measuredSize = node->measure(
        innerWidth,
...
static float calculateAvailableInnerDimension(
  ...
  if (yoga::isDefined(availableInnerDim)) {
    ...
    const float minInnerDim = minDimensionOptional.isUndefined()
        ? 0.0f
        : minDimensionOptional.unwrap() -
            paddingAndBorder; // content-box: don't subtract paddingAndBorder
...
static float distributeFreeSpaceSecondPass(
  ...
  for (size_t childIndex = flexLine.startOfLineIndex;
       childIndex < flexLine.endOfLineIndex;
       ++childIndex) {
    Node* currentLineChild = node->getChild(childIndex);
    if (currentLineChild->style().display() == Display::None ||
        currentLineChild->style().positionType() == PositionType::Absolute) {
      continue;
    }
    ...
    float childCrossSize = undefined;
    ...
    if (childStyle.aspectRatio().isDefined()) {
      ...
      childCrossSize =
          currentLineChild->getResolvedDimension(dimension(crossAxis))
              ...
              .unwrap() +
          marginCross; // content-box: add paddingAndBorder for axis
...
static void justifyMainAxis(
  ...
  Node* lastInLine = nullptr;
  for (size_t index = flexLine.endOfLineIndex;
       index > flexLine.startOfLineIndex;
       --index) {
    Node* child = node->getChild(index - 1);
    if (child->style().positionType() != PositionType::Absolute &&
  ...
  if (sizingModeMainDim == SizingMode::FitContent &&
    ...
    if (style.minDimension(dimension(mainAxis)).isDefined() &&
      ...
      const float minAvailableMainDim = style.minDimension(dimension(mainAxis))
          ...
          trailingPaddingAndBorderMain; // content-box: don't subtract
                                        // paddingAndBorder
  ...
  if (flexLine.numberOfAutoMargins == 0) {
    switch (justifyContent) {
      ...
      case Justify::SpaceBetween:
        if (flexLine.itemsInFlow > 1) {
          betweenMainDim += flexLine.layout.remainingFreeSpace /
              static_cast<float>(flexLine.itemsInFlow - 1);
        }
        break;
      ...
      case Justify::SpaceAround:
        ...
        leadingMainDim = 0.5f * flexLine.layout.remainingFreeSpace /
            static_cast<float>(flexLine.itemsInFlow);
...
//      the node or undefined if the size is not available; interpretation
...
static void calculateLayoutImpl(
  ...
  for (; endOfLineIndex < childCount;
    ...
    if (sizingModeMainDim != SizingMode::StretchFit) {
      ...
      const float minInnerWidth =
          ...
          paddingAndBorderAxisRow; // content-box: don't subtract
                                   // paddingAndBorder
      ...
      const float minInnerHeight =
          ...
          paddingAndBorderAxisColumn; // content-box: don't subtract
                                      // paddingAndBorder
      ...
      if (yoga::isDefined(minInnerMainDim) &&
        ...
        bool useLegacyStretchBehaviour = hasErrata(Errata::StretchFlexBasis);
  ...
  const bool scrollMain = isMainAxisRow ? scrollRow : scrollColumn;
  const bool scrollCross = isMainAxisRow ? scrollColumn : scrollRow;

  // If the user didn't specify a width or height for the node, set the
  // dimensions based on the children.
  if ((!scrollMain && sizingModeMainDim == SizingMode::MaxContent) ||
      (!scrollMain && sizingModeMainDim == SizingMode::FitContent)) {
  ...
  } else if (sizingModeMainDim == SizingMode::FitContent && scrollMain) {
    node->setLayoutMeasuredDimension(
        boundAxisWithinMinAndMax(
            node, mainAxis, FloatOptional(0.f), mainAxisownerSize)
            .unwrapOrDefault(0.f),
        dimension(mainAxis));
  ...
  if ((!scrollCross && sizingModeCrossDim == SizingMode::MaxContent) ||
      (!scrollCross && sizingModeCrossDim == SizingMode::FitContent)) {
    // Clamp the size to the min/max size, if specified, and make sure it
    // doesn't go below the padding and border amount.
    node->setLayoutMeasuredDimension(
        boundAxis(
  ...
  } else if (sizingModeCrossDim == SizingMode::FitContent && scrollCross) {
    // node->setLayoutMeasuredDimension(
    //     yoga::maxOrDefined(
    //         yoga::minOrDefined(
    //             availableInnerCrossDim + paddingAndBorderAxisCross,
    ...
    node->setLayoutMeasuredDimension(
        boundAxisWithinMinAndMax(
            node, crossAxis, FloatOptional(0.f), crossAxisownerSize)
            .unwrapOrDefault(0.f),
        dimension(crossAxis));
  }
  // if (scrollCross) {
